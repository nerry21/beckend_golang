...
type TripDTO struct {
	ID    int64 `json:"id"`
	Day   int   `json:"day"`
	Month int   `json:"month"` // API boleh 0..11 atau 1..12 (kita simpan DB 1..12)
	Year  int   `json:"year"`
...
func normalizeMonthToDB(m int) int {
	// terima 0..11 dari UI => simpan 1..12
	if m >= 0 && m <= 11 {
		return m + 1
	}
	// kalau sudah 1..12 biarkan
	if m >= 1 && m <= 12 {
		return m
	}
	return 1
...
func CreateTrip(c *gin.Context) {
	...
	monthDB := normalizeMonthToDB(t.Month)
	t.Month = monthDB
...
func DeleteTrip(c *gin.Context) {
	...
	if _, err := config.DB.Exec(`DELETE FROM trips WHERE id=?`, id64); err != nil {
