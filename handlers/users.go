...
func UpdateUser(c *gin.Context) {
	...
	var input struct {
		...
		Email    string `json:"email"`
		Phone    string `json:"phone"`
		Password string `json:"password"` // optional, kalau diisi kita update password_hash
		Role     string `json:"role"`
		Status   string `json:"status"`
	}
	if err := c.ShouldBindJSON(&input); err != nil {
	...
	// Ambil data lama dulu, supaya field kosong bisa pakai nilai lama
	var existing User
	err = config.DB.QueryRow(`
		SELECT id, name, username, email, phone, role, status, created_at
		FROM users WHERE id = ?
	...
	// Pakai nilai baru kalau diisi, kalau tidak pakai nilai lama
	name := existing.Name
	if input.Name != "" {
		name = input.Name
	}
	...
	// Kalau password diisi, update password_hash juga
	if input.Password != "" {
		hash, err := bcrypt.GenerateFromPassword([]byte(input.Password), bcrypt.DefaultCost)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "gagal meng-hash password: " + err.Error()})
