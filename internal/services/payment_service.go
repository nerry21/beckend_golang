...
func (s PaymentService) ValidatePayment(bookingID int64, raw json.RawMessage) error {
	...
	if err := s.BookingRepo.UpdatePaymentStatus(bookingID, "Lunas", readMethod(raw)); err != nil {
		utils.LogEvent(s.RequestID, "payment", "validate", "update booking status warning: "+err.Error())
	}
	...
	if isReturnTrip(tripRole) {
		...
		// sinkron penumpang lebih awal agar e-ticket per seat siap
		s.PassengerSvc.RequestID = s.RequestID
		_ = s.PassengerSvc.SyncFromReturn(ret)
		return nil
	}
	...
	s.PassengerSvc.RequestID = s.RequestID
	_ = s.PassengerSvc.SyncFromDeparture(dep)
	return nil
}
