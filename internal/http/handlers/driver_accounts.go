...
func GetDriverAccounts(c *gin.Context) {
	...
	vtSel := "''"
	if intdb.HasColumn(intconfig.DB, "driver_accounts", "vehicle_type") {
...
func CreateDriverAccount(c *gin.Context) {
	...
	res, err := intconfig.DB.Exec(`INSERT INTO driver_accounts (`+strings.Join(cols, ",")+`) VALUES (`+strings.Join(ph, ",")+`)`, vals...)
	if err != nil {
	...
	_ = intconfig.DB.QueryRow("SELECT COALESCE(created_at, '') FROM driver_accounts WHERE id = ?", id).
...
func UpdateDriverAccount(c *gin.Context) {
	...
	if _, err = intconfig.DB.Exec(`UPDATE driver_accounts SET `+strings.Join(setParts, ",")+` WHERE id = ?`, args...); err != nil {
...
func DeleteDriverAccount(c *gin.Context) {
	...
	res, err := intconfig.DB.Exec(`DELETE FROM driver_accounts WHERE id = ?`, id)
	if err != nil {
...
func lookupDriverVehicleType(name string) string {
	var vt string
	_ = intconfig.DB.QueryRow(`SELECT COALESCE(vehicle_type,'') FROM drivers WHERE name=? LIMIT 1`, strings.TrimSpace(name)).Scan(&vt)
