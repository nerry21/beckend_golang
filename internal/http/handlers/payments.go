...
func settingExistsForBooking(tx *sql.Tx, table string, bookingID int64, role string) (bool, error) {
	...
	if !intdb.HasTable(tx, table) {
		return false, nil
	}

	bookingCol := ""
	if intdb.HasColumn(tx, table, "booking_id") {
		bookingCol = "booking_id"
	} else if intdb.HasColumn(tx, table, "reguler_booking_id") {
	...
	if role != "" && intdb.HasColumn(tx, table, "trip_role") {
		var one int
		err := tx.QueryRow(
