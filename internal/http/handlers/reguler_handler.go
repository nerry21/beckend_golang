...
func GetRegulerQuote(c *gin.Context) {
	...
	`
	var taken string
	err = intconfig.DB.QueryRow(q, args...).Scan(&taken)
...
func GetRegulerSuratJalan(c *gin.Context) {
	...
	withBookingPhone := hasColumn(intconfig.DB, "bookings", "passenger_phone")
	withPaymentStatus := hasColumn(intconfig.DB, "bookings", "payment_status")
	...
	if scope == "trip" {
		...
		for _, b := range bookings {
			...
			if hasTable(intconfig.DB, "booking_passengers") {
				order := bestOrderBy(intconfig.DB, "booking_passengers")

				withPassengerPhone := hasColumn(intconfig.DB, "booking_passengers", "passenger_phone")
				...
				rows, qerr := intconfig.DB.Query(q, b.ID)
				if qerr != nil {
			...
			if !gotAny && hasTable(intconfig.DB, "booking_seats") {
				order := bestOrderBy(intconfig.DB, "booking_seats")
				q := `
					SELECT seat_code
					FROM booking_seats
					WHERE booking_id = ?
	...
	if hasTable(intconfig.DB, "booking_passengers") {
		...
		rows, qerr := intconfig.DB.Query(q, id64)
		if qerr != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"message": "gagal query booking_passengers: " + qerr.Error()})
			return
		}
	...
	if len(passengers) == 0 && hasTable(intconfig.DB, "booking_seats") {
		order := bestOrderBy(intconfig.DB, "booking_seats")
		q := `
			SELECT seat_code
			FROM booking_seats
